...
Both of these plots show the distribution of a numeric variable (ocean bottom temperature in °C). **How would you interpret each of these?**
...
## {#real-world-examples data-menu-title="Real-world examples"} 

TODO add real world examples of a histogram and a density plot for students to discuss / interpret

. . . 

::: {.center-text .body-text-m .teal-text}
Let's build and modify some histograms & density plots using `{ggplot2}`'s `geom_histogram()` and `geom_density()` geoms.
:::
...
Do you *need* all (12) groups? Or can you share the most relevant data using fewer groups? Let's compare just three months: April (generally the coldest month), October (generally a hot month), and June (somewhere in between). We'll also manually update the colors:
...
*Why are the months still in chronological order, despite reordering them using `mutate()`?*
:::
...
[Modify bin / band widths]{.slide-title}
...
[Modify bandwidth by declaring a multiplier of the default bandwidth adjustment. Reducing the `adjust` argument reduces the amount of smoothing (default `adjust = 1`):]{.body-text-s}
...
[Combining histogram & density geoms]{.slide-title}
...
```{r boxyplot-hist-comparison}
#| eval: true
#| echo: false
#| out-width: "100%"
#| fig-align: "center"
knitr::include_graphics("images/week2/boxplot-quartiles.png")
```
...
TODO add example for students to practice interpreting
...
[Dodge when you have an additional grouping varialbe]{.slide-title2}
...
[Violin plots *summarize* data, meaning they don't show the underlying shape of the distribution or sample size (though jittered points can be added, if appropriate). They provide a compact summary of a dataset's center, spread and indications of skewness, and allow many groups to be compared side-by-side while remaining readable.]{.body-text-s}
