...
[Using the publicly-available CDC data on Lyme disease and population estimates from the [US Census Bureau](https://www.census.gov/){target="_blank"} (via the [`{tidycensus}` package](https://walker-data.com/tidycensus/){target="_blank"}), we'll explore **changes in Lyme disease incidence (cases/100k people) over time**, by state.]{.body-text-s}
