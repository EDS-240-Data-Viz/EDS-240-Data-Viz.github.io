...
Let's say we want to **explore the change in average SUI through time for each of the ten HUC 18 subregions**. We can use a heatmap to create a matrix that displays average SUI (numeric, continuous) by subregion (categorical) and year (categorical):
...
come back to this, these data seem really off but maybe it's becuase I'm not understanding availb_mm_mo and consum_mm_mo...

consider doing so for Central California subbasins?
...
Let's say we want to **compare overall average water availability and consumption by subregion from 2010–2020.** We can use a dumbbell (aka Cleveland) plot to display the average amount of water (numeric) available and consumed (categroical), by subregion (categorical):
