...
#| fig-asp: 0.7
#| out-width: "100%"
plot <- ggplot(sbc_subbasin_monthly) +
  geom_linerange(aes(y = month,
                     xmin = mean_consum, xmax = mean_avail)) + 
  geom_point(aes(x = mean_avail, y = month), 
             color = "#448F9C", 
...
ggplot(sbc_subbasin_monthly) +
  ...
  geom_point(aes(x = mean_consum, y = month), 
             ...
             stroke = 2,
             size = 5.5) +
  labs(title = "Water supply vs. demand across the Santa Barbara Coastal\nsubbasin (2010-2020)",
...
#| eval: false
#| echo: true
#| code-line-numbers: "21,25,29,32,36"
ggplot(sbc_subbasin_monthly) +
  geom_linerange(aes(y = month,
                     xmin = mean_consum, xmax = mean_avail)) + 
  geom_point(aes(x = mean_avail, y = month), 
...
ggplot(sbc_subbasin_monthly) +
  ...
  geom_point(aes(x = mean_consum, y = month), 
             ...
             size = 5.5) +
  labs(title = "Water supply vs. demand across the Santa Barbara Coastal\nsubbasin (2010-2020)",
       subtitle = "Summer deficits exist, despite winter water surplus", # Winter water availability exceeds demand, but summer months experience deficits
       caption = caption,
       x = "Mean monthly water supply & demand (mm)") +
  theme_minimal() +
  theme(
...
::: {.center-text}
**Note:** Text rendered with `{ggtext}` doesn't always appear correctly in the plots pane (or when using the Zoom window) -- see this separate, but potentially related [GitHub issue](https://github.com/wilkelab/ggtext/issues/84){target="_blank"}.
:::
...
[Rather than a traditional legend, let's **color-code our title text (i.e. [supply]{style="color:#448F9C"} & [demand]{style="color:#9C7344"}) to match the points in our plot**. We can again use the [`{ggtext}` package](https://wilkelab.org/ggtext/){target="_blank"} to apply simple Markdown and HTML rendering to our ggplot text.]{.body-text-s}

. . . 

[We'll need to both **format our title using Markdown and some HTML**, and **update `theme()`** so that this text element is rendered using `ggtext::element_textbox()` (just as we did with our caption). Let's bold and color the words "supply" and "demand":]{.body-text-s}
