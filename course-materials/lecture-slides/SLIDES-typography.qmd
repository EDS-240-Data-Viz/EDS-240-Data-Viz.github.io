...
<!-- CODE RENDERS ON SLIDE BUT DOES NOT RUN -->

```{r dumbbell-original-wrangle-echo-true}
#| eval: false
#| echo: true
...
<!-- CODE RUNS BUT DOES NOT RENDER -->

```{r dumbbell-original-wrangle-eval-true}
#| eval: true
#| echo: false
#| out-width: "50%"
#| fig-align: "center"
...
[Use relative font sizes within your theme! If you need to make your plot larger for a presentation, for example, you'll just need to increase your `base_size` and all text scales proportionally. Resulting plot is rendered on next slide.]{.body-text-s}
...
ggplot(sbc_subbasin_monthly) +
  ...
  theme(
    ...
    plot.title = element_text(face = "bold",
                              size = rel(0.98),#22,
                              color = pal["dark_gray"]),
    plot.subtitle = element_text(size = rel(0.9),#16,
                                 color = pal["light_gray"],
                                 margin = margin(t = 3, b = 8)),
    axis.text = element_text(size = rel(0.7),#13,
                             color = pal["light_gray"]),
    axis.title.x = element_text(size = rel(0.75),#14,
                                margin = margin(t = 15)),
    axis.title.y = element_blank(),
    plot.caption = element_text(face = "italic",
                                size = rel(0.5), #none
...
[Enable `{showtext}` & apply Google Fonts]{.slide-title}
...
#| eval: false
#| echo: true
#| code-line-numbers: "1-2,26,30,34,37,41,50-51"
...
[[Font Awesome](https://fontawesome.com/){target="_blank"} is a library of icons, which can be imported and used similar to Google Fonts. You'll need to download the font files first (see today's [pre-class prep instructions](../week7.qmd#pre-class-prep){target="_blank"}). We can then use `font_add()` to make them available for use in our ggplots:]{.body-text-s}
...
[We'll need to both **format our title using some HTML**, *and* **update `theme()`** so that this text element is rendered using `ggtext::element_markdown()` (like our caption). Let's construct our title using `glue::glue()`:]{.body-text-s}
...
[Then update to `element_markdown()` and increase the `lineheight` to give the two lines a bit of breathing room:]{.body-text-s}

```{r lineheight}
#| eval: false
#| echo: true
#| code-line-numbers: "1,4"
theme(plot.title = ggtext::element_markdown(family = "sarala",
                                            face = "bold",
                                            size = rel(0.98),
                                            lineheight = 1.2,
                                            color = pal["dark_gray"]))
