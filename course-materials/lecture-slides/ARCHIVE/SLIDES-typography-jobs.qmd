...
[Week 6 | February 10^th^, 2025]{.custom-subtitle3}

[*Archive (job data) | see [latest version](../SLIDES-typography.qmd) of this lesson*]{.custom-subtitle3}
...
Source: [The UX Designer’s Guide to Typography](https://www.interaction-design.org/literature/article/the-ux-designer-s-guide-to-typography){target="_blank"}
:::
...
{{< include ../sections/ARCHIVE/6.1-dumbbell-original.qmd >}}
...
This code should look familiar (copied from the [amounts / rankings lecture](SLIDES-amounts-rankings-jobs.qmd){target="_blank"}):
...
This code was copied from the [amounts / rankings lecture](SLIDES-amounts-rankings-jobs.qmd){target="_blank"}. Resulting plot is rendered on the next slide!
...
[In the [colors lecture](../SLIDES-colors.qmd){target="_blank"}, we learned **to be predictable in our color choices, but to avoid stereotypes** ([Muth 2018](https://blog.datawrapper.de/colors/){target="_blank"}). It can be helpful for readers to choose a cool color for men (e.g. blues / purples) and warmer color for women (e.g. yellows, oranges, warm greens).]{.body-text-s}
...
[The primary purpose of the [`{monochromeR}` package](https://github.com/cararthompson/monochromeR){target="_blank"} is for creating monochrome color palettes, however, it also includes a **helpful function for viewing our palette**:]{.body-text-s}
...
<!-- FOR DISPLAYING PLOT ONLY (NO CODE SHOWN ON SLIDE)  -->
...
ggplot(subset_jobs) +
  ...
  theme(
    ...
    plot.title = element_text(face = "bold",
                              size = 19, # 18
                              color = earnings_pal["dark_text"]),
    plot.subtitle = element_text(size = 14, #11.5
                                 color = earnings_pal["light_text"],
                                 margin = margin(t = 2, r = 0, b = 6, l = 0)),
    plot.caption = element_text(face = "italic",
                                color = earnings_pal["light_text"],
...
#| out-width: "100%"
#| fig-align: "center"
knitr::include_graphics(here::here("course-materials", "images", "lecture-slides", "google-fonts.gif"))
```
...
#| out-width: "100%"
#| fig-align: "center"
knitr::include_graphics(here::here("course-materials", "images", "lecture-slides", "josefin-sans.png"))
```
:::
...
::: {.center-text}
**[Sen](https://fonts.google.com/specimen/Sen?query=sen){target="_blank"}**
:::
...
#| out-width: "100%"
#| fig-align: "center"
knitr::include_graphics(here::here("course-materials", "images", "lecture-slides", "sen.png"))
```
:::
...
::: {.footer}
**Important:** Pick typefaces that have all the *font styles* (e.g. bold, italic) that you want to use. [Sen](https://fonts.google.com/specimen/Sen?query=sen){target="_blank"} does *not* come in an italic style, meaning we won't be able to render any text in the typeface Sen and the font style italic (demonstrated in the next slides). 
:::
...
#| eval: false
#| echo: true
#| code-line-numbers: "19,23,27,31,36"
ggplot(subset_jobs) +
  geom_linerange(aes(y = occupation,
                     xmin = total_earnings_female, xmax = total_earnings_male)) + 
  geom_point(aes(x = total_earnings_male, y = occupation), 
...
[[Font Awesome](https://fontawesome.com/){target="_blank"} is a library of icons, which can be imported and used similar to Google Fonts. You'll need to download the font files first (see today's [pre-class prep instructions](../../week6.qmd#pre-class-prep){target="_blank"}). We can then use `showtext::font_add()` to make them available for use in our ggplots:]{.body-text-s}
...
Let's say I want to include my GitHub username along with the GitHub icon in the caption of my plot. Start by searching the [**Free** icons on Font Awesome](https://fontawesome.com/search?o=r&m=free){target="_blank"} and click on the one you want to use (here, the [**github**](https://fontawesome.com/icons/github?f=brands&s=solid){target="_blank"} icon). Find the icon's Unicode in the top right corner of the popup box:
...
[To use this unicode in HTML, we need to stick a `&#x` ahead of it. We can make our lives a bit easier by saving our unicode (as well as our username text) to variable names. We'll then use the [`glue::glue()` function](https://glue.tidyverse.org/) to construct our full caption. Importantly, `glue()` will evaluate expressions enclosed by braces as R code.]{.body-text-s}
...
We can do this the same way that we constructed our caption -- **note** that this money icon is from the [`fa-regular` family](https://fontawesome.com/icons/money-bill-1?f=classic&s=regular){target="_blank"} (though you could choose to use the [solid version](https://fontawesome.com/icons/money-bill-1?f=classic&s=solid){target="_blank"} as well):
...
[Apply our new caption & subtitle]{.slide-title}
...
There are a few options, all which replace `{ggplot2}`'s  `element_text()` -- be sure to check out the documentation as you're deciding which to use:
  ...
  - [`element_markdown()`]{.body-text-s}
  - [`element_textbox()`]{.body-text-s}
  - [`element_textbox_simple()`]{.body-text-s}
  
---

## {#update-theme-elements-code data-menu-title="Update theme elements (code)"} 

[Update `theme()` elements to render styling]{.slide-title}
...
::: {.center-text}
**Note:** Text rendered with `{ggtext}` doesn't appear correctly in the RStudio plots pane (or when using the Zoom window) -- see this separate, but potentially related [GitHub issue](https://github.com/wilkelab/ggtext/issues/84).
:::
...
Rather than a traditional legend, let's **color-code our subtitle text (i.e. [male]{style="color:#2D7787"} & [female]{style="color:#FC6B4B"}) to match the points in our plot**. We can again use the [`{ggtext}` package](https://wilkelab.org/ggtext/){target="_blank"} to apply simple Markdown and HTML rendering to our ggplot text.

<br>

. . . 

We already have the correct `theme()` elements added to our plot, but we do need to **update our title text with Markdown and HTML styling**. Let's color the words "male" and "female":
...
title <- glue::glue("<span style='color:#2D7787;'>Males</span> earn more than <span style='color:#FC6B4B;'>females</span> across most occupations")
```
...
## {#updated-text data-menu-title="Voilà!"} 
...
ggplot(subset_jobs) +
  ...
  scale_x_continuous(labels = scales::label_currency(scale = 0.001, suffix = "k"),
                     breaks = c(25000, 50000, 75000, 100000, 125000)) +
  labs(title = title,#"Males earn more than females across most occupations",
       subtitle = subtitle,
       caption = caption) +
  theme_minimal() +
  theme(
    plot.title.position = "plot",
    plot.title = ggtext::element_textbox(family = "josefin",
                              face = "bold",
                              size = 19, # 18
                              color = earnings_pal["dark_text"]),
    plot.subtitle = ggtext::element_textbox(family = "sen",
...
#| out-width: "100%"
#| fig-align: "center"
knitr::include_graphics(here::here("course-materials", "images", "lecture-slides", "salary_plot_8x7.png"))
```
