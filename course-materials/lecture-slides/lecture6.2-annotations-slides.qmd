...
::: {.footer}
Read these two *great* posts: [What to consider when using text in data visualizations](https://blog.datawrapper.de/text-in-data-visualizations/){target="_blank"} & [Respect your readers’ time](https://blog.datawrapper.de/readers-time/){target="_blank"}, both by Lisa Charlotte Muth
:::
...
# lobs <- read_csv(here::here("week6", "data", "metabolism-foraging-data.csv")) |> 
  # mutate(temp = as.factor(temp))
# lobs <- read_csv(here::here("course-materials", "lecture-slides", "data", "metabolism-foraging-data.csv")) |> 
#   mutate(temp = as.factor(temp))
...
Adapted from [Csik et al. 2023](https://doi.org/10.3389/fmars.2023.1072807){target="_blank"}, [Figure 5](https://www.frontiersin.org/files/Articles/1072807/fmars-10-1072807-HTML/image_m/fmars-10-1072807-g005.jpg){target="_blank"}
:::
...
# mono <- read_csv(here::here("course-materials", "lecture-slides", "data", "mono.csv")) 
...
::: {.center-text .body-text-s .gray-text}
Borrowed from Allison Horst's [Customized Data Visualization in `{ggplot2}`](https://allisonhorst.github.io/eds221-day8-comp/data_viz.html){target="_blank"} materials
:::
...
[Alternatively, `annotate()` requires that we define a `geom` type (e.g. `"text"`, `"rect"`). We can also omit the `show.lengend` argument, since `annotate()` doesn't produce a legend.]{.body-text-s}
...
- [the [`{ggforce}` package](https://ggforce.data-imaginist.com/) has *so* many awesome functions, including a series of [annotation functions](https://ggforce.data-imaginist.com/reference/index.html#annotation){target="_blank"} (e.g. check out [`geom_mark_ellipse()`](https://ggforce.data-imaginist.com/reference/geom_mark_ellipse.html){target="_blank"}, demoed in [lecture 5.1](lecture5.1-good-viz-slides.qmd#ggforce){target="_blank"}), and [`facet_zoom()`](https://ggforce.data-imaginist.com/reference/facet_zoom.html){target="_blank"}, for zooming into a subset of data; [Tuo Wang](https://tuowang.rbind.io/) also has some great examples in [this blog post](https://tuowang.rbind.io/blog/ggplot2-extensions/02-ggplot2-extensions/#3-span-stylecolor-coralggforcespan-package){target="_blank"}]{.body-text-s}
...
- [[4 Ways to use colors in ggplot more efficiently](https://albert-rapp.de/posts/ggplot2-tips/07_four_ways_colors_more_efficiently/07_four_ways_colors_more_efficiently#label-directly){target="_blank"}, by [Albert Rapp](https://albert-rapp.de/) (includes some great annotation examples){target="_blank"}]{.body-text-s}
