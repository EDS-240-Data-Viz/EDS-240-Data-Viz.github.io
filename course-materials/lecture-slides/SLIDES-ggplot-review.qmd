...
::: {.incremental}
- super flexible, layered plot specification (see [Wickham 2008](http://vita.had.co.nz/papers/layered-grammar.pdf){target="_blank"}), based on an underlying "grammar of graphics" ([Wilkinson 2005](https://link.springer.com/book/10.1007/0-387-28695-0){target="_blank"}, originally published in 1999)
...
#| fig-align: "center"
#| out-width: "60%"
knitr::include_graphics(here::here("course-materials", "images", "lecture-slides", "ggplot2.png"))
```
:::
...
adapted from [Cédric Scherer's](https://www.cedricscherer.com/){target="_blank"} rstudio::conf(2022) workshop, [Graphic Design with ggplot2](https://rstudio-conf-2022.github.io/ggplot2-graphic-design/){target="_blank"}
:::
...
[{{< fa chart-line title="a line chart" >}} graphic layers]{.slide-title}
...
**1. data** -- in [tidy](https://doi.org/10.18637/jss.v059.i10){target="_blank"} format **+** define aesthetics (how variables map onto a plot e.g. axes, shape, color, size)  
...
knitr::include_graphics(here::here("course-materials", "images", "lecture-slides", "horst-tidy-data-friends.png"))
```
...
knitr::include_graphics(here::here("course-materials", "images", "lecture-slides", "horst-tidy-data-definition.png"))
```
...
knitr::include_graphics(here::here("course-materials", "images", "lecture-slides", "horst-untidy-data.png"))
```
...
::: {.center-text}
This tibble is in an **untidy** (aka **wide**) format.
:::
...
```{r untidy-temps-table}
#| eval: true
#| echo: false
#| fig-align: "center"
#| out-width: "50%"
knitr::include_graphics(here::here("course-materials", "images", "lecture-slides", "untidy-multi-obs-per-row.png"))
```

<br>

::: {.center-text .gray-text}
Want more examples of untidy data? Check out these [teaching materials](https://learning.nceas.ucsb.edu/2023-09-ucsb-faculty/session_07.html#tidy-data){target="_blank"} from the NCEAS Learning Hub showcasing real-world examples of *very* untidy data.
:::
...
We can use `tidyr::pivot_longer()` to "lengthen" our data, aka convert it from untidy (wide) to **tidy (long):**
...
knitr::include_graphics(here::here("course-materials", "images", "lecture-slides", "horst-workbench.png"))
```
...
#| fig-align: "center"
#| out-width: "70%"
knitr::include_graphics(here::here("course-materials", "images", "lecture-slides", "core-tidyverse.png"))
```

::: {.center-text .gray-text}
Check out the [tidyverse website](the packages that you’re likely to use in everyday data analyses){target="_blank"} to learn more about each of these packages
:::
...
#| fig-align: "center"
#| out-width: "55.5%"
knitr::include_graphics(here::here("course-materials", "images", "lecture-slides", "r4ds-cover.png"))
```

::: {.center-text .gray-text}
The best resource for learning all things [R for Data Science](https://r4ds.hadley.nz/){target="_blank"}!
:::
...
::: {.center-text}
**Let's make some ggplots using data from [`{palmerpenguins}`](https://allisonhorst.github.io/palmerpenguins/){target="_blank"} (which are already tidy)!**
:::
...
knitr::include_graphics(here::here("course-materials", "images", "lecture-slides", "palmer-penguins.png"))
```

::: {.center-text .gray-text}
*Artwork by [Allison Horst](https://allisonhorst.github.io/palmerpenguins/articles/art.html){target="_blank"}*
:::

::: {.footer}
[*The examples on the following slides were adapted from R for Data Science, Chapters [2](https://r4ds.hadley.nz/data-visualize){target="_blank"} and [10](https://r4ds.hadley.nz/layers){target="_blank"}.*]{.gray-text .center-text}
:::
...
[**1. data** -- in [tidy](https://doi.org/10.18637/jss.v059.i10){target="_blank"} format + define aesthetics (how variables map onto a plot e.g. axes, shape, color, size)]{.body-text-s}  
...
Next, we'll layer on a geometric object (aka geom) that our plot will use to represent our penguin data. There are many geoms (`geom_*()`) that are [built into `{ggplot2}` already](https://ggplot2.tidyverse.org/reference/#geoms){target="_blank"} (and more when you use extension packages). To create a scatter plot:
...
[If we'd like to represent species using another aesthetic (e.g. color, shape, size), we need to modify our plot's aesthetic (i.e. inside `aes()`) -- **any time we want to modify the appearance of our plotted data based on a variable in our data set, we do so within `aes()`.** This process is known as **scaling**. A legend will automatically be added to indicate which values (in this case, colors) correspond to which level of our variable (in this case, species):]{.body-text-s}
...
## {#color-points-by-mass data-menu-title="Color points by body mass"} 

[{{< fa palette title="a painter's palette" >}} Use color to describe a continuous variable]{.slide-title2}
...
## {#piping-in data-menu-title="Piping into a ggplot"}

[{{< fa chart-line title="a line chart" >}} Piping directly into a ggplot]{.slide-title}
...
[{{< fa question title = "question-mark" >}} What is a statistical transformation?]{.slide-title}
...
[Let's say we have a data frame with calculated count values (e.g. `penguins_summary`) that we'd like to plot using `geom_bar()`. We can change `stat = "count"` (default) to `stat = "identity"` to generate bar heights based off the "identity" of values in the `n` column of `penguin_summary`.]{.body-text-s}

```{r stat-identity-viz2}
#| eval: true
#| echo: true
#| fig-align: "center"
...
[Now let's say we'd like to display the same bar chart with y-axis values as proportions, rather than counts. We can override the default mapping of transformed variables to aesthetics with:]{.body-text-s}
...
- [first, `stat = "count"` counts the number of occurrences in each category (here, `species`)]{.body-text-s}
- [next, specifying `y = after_stat(prop)` tells ggplot to calculate the proportion of each `species` count relative to the total count (delayed until after counts have been computed)]{.body-text-s}
- [`group = 1` ensures that proportions are calculated across *all* species -- default behavior of `geom_bar()` is to group by the `x` variable  to separately count the number of rows in each level (`Adelie`, `Chinstrap`, `Gentoo`), but we actually need to know how many *total* penguins there are in the data set in order to calculate proportions]{.body-text-s}
...
[{{< fa question title = "question-mark" >}} What is a position adjustment?]{.slide-title}
...
[{{< fa chart-line title="a line chart" >}} The default position for `geom_bar()` is "stack"]{.slide-title2}
...
## {#pos-alt data-menu-title="Alternative ways to define positions"} 

[{{< fa chart-line title="a line chart" >}} Alternatively, use `position = position_*()`]{.slide-title3}
...
[{{< fa question title = "question-mark" >}} What is a coordinate system?]{.slide-title}
...
`{ggplot2}` comes with a number of [complete themes](https://ggplot2.tidyverse.org/reference/ggtheme.html){target="_blank"}, which control all non-data display. See two examples below:
...
Further modify nearly any non-data element of your plot using [`theme()`](https://ggplot2.tidyverse.org/reference/theme.html){target="_blank"}.
...
::: {.center-text}
Let's combine a few of our previous separate plots into a single graphic to round out our quick `{ggplot2}` review. We'll do so using the **[`{patchwork}` package](https://patchwork.data-imaginist.com/){target="_blank"}**
:::
...
## {#save-plot-objs data-menu-title="Save plots to object names"} 

[First, save your plots to object names]{.slide-title}
...
p1 <- ggplot(penguins, aes(x = bill_length_mm, y = bill_depth_mm, color = body_mass_g)) +
  geom_point() +
  scale_color_gradient(low = "#F7DD4C", high = "#132B43") +
  theme(legend.position = "top") # note: this is newly added from earlier!
...
## {#patchwork data-menu-title="Combine with patchwork"} 

[Then, combine with `{patchwork}`]{.slide-title}
...
## {#stacked data-menu-title="Patchwork stacked"} 
...
## {#side-stacked data-menu-title="Patchwork 3 plots"} 
...
[Building a data viz is an iterative process!]{.slide-title}

<hr>

[We'll spend the next two weeks learning how to build some basic fundamental charts and talking about important considerations when choosing a graphic form for presenting your data. Then, we'll move into graphic design theory and the tools and packages in the `{ggplot2}` ecosystem that make it possible.]{.body-text-s}
...
#| fig-align: "center"
#| out-width: "60%"
knitr::include_graphics(here::here("course-materials", "images", "lecture-slides", "scherer-evo-ggplot.gif"))
```

::: {.center-text .gray-text .body-text-s}
*Visualization by [Cédric Scherer](https://www.cedricscherer.com/top/about/){target="_blank"}, from his blog post, [The Evolution of a ggplot (Ep.1)](https://www.cedricscherer.com/2019/05/17/the-evolution-of-a-ggplot-ep.-1/){target="_blank"} -- create your own ggplot evolution gif using the [`{camcorder}` package](https://github.com/thebioengineer/camcorder){target="_blank"}*
:::
