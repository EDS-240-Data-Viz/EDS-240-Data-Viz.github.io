...
mko_clean <- mko |>
  ...
  # (not totally necessary for our plots, but helpful to know!)
  mutate(
  date_time = make_datetime(year, month, day, tz = "America/Los_Angeles") + # find tz identifiers: https://en.wikipedia.org/wiki/List_of_tz_database_time_zones
              seconds(decimal_time * 86400)  # decimal_time is fraction of day; 86400 seconds in a day
  ) |>
...
::: {.notes}
If `decimal_time = 0.5` (noon):

- `0.5 * 86400 = 43200` seconds
- `seconds(43200)` creates a duration of 43200 seconds (12 hours)
- Adding it to midnight gives you 12:00:00 that same day
:::
...
Histograms and density plots are often talked about as somewhat interchangeable. Below, they're both used to show the distribution of a numeric variable (ocean bottom temperature in °C). 
...
::: {.center-text .body-text-s}
Smoothed versions of a histogram, where the y-axis represents density (i.e. how concentrated your data is at each value). Taller parts of the curve indicate more common values, and shorter parts indicate rarer values. The area under the entire curve always equals 1, representing 100% of your data.
...
::: {.footer}
Check out [this cool interactive tool](https://mathisonian.github.io/kde/){target="_blank"}, by [Matthew Conlen](https://mathisonian.com/){target="_blank"}, for a visual explanation of Kernal Density Estimate (KDE). KDE is the method used for visualizing the distribution in a density plot.
:::
...
[Modify `binwidth` (30 `bins` by default) – does a bin width of 1 (°C) actually make sense? Consider scale of interest. Also be mindful when using bins – too few will result in loss of distribution shape, too many adds noise.]{.body-text-s}
...
::: {.footer}
`geom_density()` relies on `density()`, which uses the `nrd0` bandwidth selector (Silverman's rule-of-thumb-style estimator). This is a reliable starting point, but you may consider other selectors based on your data. Check out [this article](https://aakinshin.net/posts/kde-bw/){target="_blank"} to read more about bandwidth selection.
:::
...
ggplot(mko_clean, aes(x = Temp_bot)) + 
  geom_histogram(aes(y = after_stat(density)), # scale hist to match density curve
                 fill = "gray", color = "black", alpha = 0.75) +
...
mko_clean |> 
  ...
  ggridges::geom_density_ridges(rel_min_height = 0.001, scale = 3)  # `rel_min_height` sets threshold for relative height of density curves (any values below threshold treated as 0); `scale` controls extent to which different densities overlap
```
:::
...
mko_clean |> 
  ...
  ggplot(aes(x = Temp_bot, y = month_name, fill = after_stat(x))) + # `after_stat(x)` tells ggplot: "Don't use the original data for coloring. Instead, wait until you've drawn the density curve, then color each part of that curve based on its x-axis position."
  ggridges::geom_density_ridges_gradient(rel_min_height = 0.001, scale = 3) + 
  scale_fill_gradientn(colors = c("#2C5374","#849BB4", "#D9E7EC", "#EF8080", "#8B3A3A"))
```
:::
...
`fill = after_stat(x)` = “Wait until you’ve computed the density ridge, then color each segment of the ridge based on its x-position.”
`after_stat(x)` uses the x-values of the smoothed density curve, not the original data points, to determine color.

What` after_stat(x)` means:
- When you make a density ridgeline plot, ggplot doesn’t draw the curve directly from your raw temperature points.
- Instead, for each month, ggplot:
1. Looks at all the Temp_bot values for that month
2. Draws a smooth curve that represents where temperatures are more or less common
3. Builds that curve out of many tiny points from cold to warm temperatures

- Those tiny points each have: an x position (how cold or warm) & a height (how tall the curve is there)
- when you write, `fill = after_stat(x)`, you're saying, *“Color each tiny piece of the density curve based on how cold or warm it is.”*
:::
...
::: {.center-text .dark-blue-text}
***What are the tradeoffs between reordering groups within a ggplot (as above) vs. during the data wrangling stage (e.g. as we did for our [histogram and density](SLIDES-distributions.qmd#histogram-density-groupnum) and [ridgeline plots](SLIDES-distributions.qmd#ggridges))?***
:::
...
## {#extra-time data-menu-title="Extra Time?"} 

[Extra time? Let's practice.]{.slide-title2}

<hr>


[TidyTuesday published a [dataset on palm traits](https://github.com/rfordatascience/tidytuesday/tree/main/data/2025/2025-03-18){target="_blank"} on March 18, 2025, sourced from the [PalmTraits 1.0 Database](https://www.nature.com/articles/s41597-019-0189-0){target="_blank"}. With your learning partners, explore these data and visualize the distribution of one or more palm traits of your choosing. Be intentional about how you group the data and whether those groupings are appropriate for the chart types you choose. Apply the visualization and design considerations discussed in lecture. Begin by importing the data directly from the tidytuesday repo:]{.body-text-s}
...
#| fig-align: "center"
#| out-width: "70%"
knitr::include_graphics(here::here("course-materials", "images", "lecture-slides", "palms-bren.png"))
```

::: {.footer}
Image source: [Bren](https://bren.ucsb.edu/masters-programs/master-environmental-science-and-management/tuition-and-finance/fellowships){target="_blank"}
:::
...
palmtrees |> 
  ...
  ggplot(aes(x = max_leaf_number, y = palm_subfamily)) +
  geom_boxplot()

# histogram ----
palmtrees |> 
  filter(palm_subfamily != "Nypoideae") |> 
  ggplot(aes(x = max_leaf_number, fill = palm_subfamily)) +
  geom_histogram(position = "identity", alpha = 0.5)
