...
We'll learn how to use [Font Awesome](https://fontawesome.com/) icons in our ggplots. To do so, you'll need to download the Font Awesome font files and save them to your class repo. Do so by following these steps:
...
2. Review setup / wrangling / base plot code and make note of any questions you might have. Due to time constraints, we won't be live-coding our way through all code in class. However, we'll reserve some time to answer any questions before we jump into data viz {{< fa face-smile title="a smiling face" >}}. Specifically:
    - Template 6.1 begins where we left off in creating our salary plot (in [lecture 4.1](lecture4.1-rankings-slides.qmd){target="_blank"}). Review the code under the `## Setup`, `## Wrangle Data` and `## Recreate original plot` sections to re-familiarize yourself.  
...
|-------------------|-------------------|--------------|
| [D6 slides: saving plots](discussion/week6/week6-slides.qmd){target="_blank"} | *see pre-discussion prep* | [D6 solution](discussion/week6/week6-solution.qmd) |
